...
# parser.add_argument("--checkpoint", type=str, default="/home/junzhewu/pohsun/SG-VLN/robot_env/spot_checkpoint.pt", help="Path to model checkpoint exported as jit.")
...
while simulation_app.is_running():
    ...
    with torch.inference_mode():
        # Policy forward pass
        command = torch.tensor([[1., base_command[1], base_command[2], 0.0]], device="cuda:0", dtype=torch.float32)
        # command = torch.tensor([[base_command[0], base_command[1], base_command[2], 0.0]], device="cuda:0", dtype=torch.float32)
        action = policy(obs)
    ...
    # Get camera data
    camera = manager_env.scene["camera"] 
