#!/bin/bash
# example usage: bash experiments/launch.sh experiment_name config_file "5,4,0,1,2,3" sample100 true
# example usage: bash experiments/launch.sh testing realworld.yaml 1 1 "0" sample100 false
...
if [ $# -lt 4 ]; then
    ...
    echo "Example: $0 bash experiments/launch.sh testing realworld.yaml 1 1 "0" sample100 false"
    exit 1
fi
...
#54.7 for real world
# Set up base command
cmd="EXP_NAME=${expname} python3 -u socket_main_2.py \
    -n ${n_process_per_gpu} \
    --num_gpu ${num_gpu} \
    --fmm_planner \
    --edge_goal \
