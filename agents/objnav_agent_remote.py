...
from constants import color_palette, category_to_id, categories_21, categories_21_plus_stairs, detector_classes, bg_classes, detector_classes_real_world
...
class ObjectNav_Agent(Agent):
    def __init__(self, args, follower=None) -> None:
        ...
        if self.args.real_world:
            self.classes = detector_classes_real_world
        elif self.args.gt_perception:
        ...
        # TODO: need real_world camera intrinsic matrix
    ...
    def reset_stair_climb_params(self, current_floor_camera_height):
        ...
        self.real_world_climb = False # only for real world experiment
    ...
    def act(self, observations: Observations, sim_info, step_data_queue, gui_prompt_queue=None, gui_viz_queue=None):
        ...
        env, scene_name, episode_id= sim_info['env'], sim_info['scene_name'], sim_info['episode_id']
        self.position = sim_info['position']
        self.rotation = sim_info['rotation']
        if self.l_step == 0:
            self.init_sim_position = copy.deepcopy(sim_info['position']) #used for transformation matrix
            self.init_sim_rotation = copy.deepcopy(quaternion.as_rotation_matrix(sim_info['rotation']))
            self.initial_pose = np.eye(4)
            self.initial_pose[:3,:3] = copy.deepcopy(quaternion.as_rotation_matrix(sim_info['rotation']))
            self.initial_pose[:3,3] = copy.deepcopy(sim_info['position'])

            self.additional = np.eye(4)
            self.additional[:3,:3] = np.array([[0,0,1],
                                    [0,-1,0],
                                    [1,0,0]])
                    

            vln_logger.info("init agent_position: "+str(self.init_sim_position))
            vln_logger.info("camera_position: "+str(self.init_sim_rotation))
            ...
            if self.args.real_world:
                self.cate_object = observations['objectgoal']
                self.text_queries = self.cate_object

                if self.cate_object not in self.classes:
                    self.classes.append(self.cate_object)
                    print(self.classes)

            elif self.args.gt_perception or self.args.task_config == 'objectnav_mp3d_rgbd.yaml':
        ...
        camera_matrix_T = self.get_transform_matrix()
        vln_logger.add_pose(camera_matrix_T)
        camera_position = camera_matrix_T[:3, 3]
        self.camera_position = camera_position #used for floors and stuff
        self.Open3D_traj.append(camera_matrix_T) #useless
        ...
        full_scene_pcd = build_full_scene_pcd(depth, image_rgb, self.args.hfov, self.args.real_world)
        ...
        if self.args.real_world:
            points = np.array(full_scene_pcd.points)
            # points[:, 1] -= 0.8
            full_scene_pcd.points = o3d.utility.Vector3dVector(points)
            print(f"[REAL_WORLD] max_x: {points[:, 0].max()}, min_x: {points[:, 0].min()}")
            print(f"[REAL_WORLD] max_y: {points[:, 1].max()}, min_y: {points[:, 1].min()}")
            print(f"[REAL_WORLD] max_z: {points[:, 2].max()}, min_z: {points[:, 2].min()}")
        
        if self.args.gradient_mapping:
            obs_i_values, obs_j_values = self.pcd_to_grid_indx(full_scene_pcd)
            self.update_map_gradient(camera_position)
        elif self.args.new_mapping:
            ###### new method that may #####
            obs_i_values, obs_j_values = self.pcd_to_grid_indx(full_scene_pcd)
        ...
        # detect frontier
        target_fmm_cost, frontier_edge_map, frontier_candidate_list = detect_frontier(self.explored_map, self.obstacle_map, self.current_grid_pose, threshold_point=8, real_world=self.args.real_world,visited_mask=self.visited_vis)
        ...
        if self.args.real_world:
            similarity_threshold = 1.0 # short circuit
        ...
        if len(self.objects) > 0:
            ...
            if self.args.gt_perception:
                ...
                for i in range(len(self.objects)):
                    ...
                    is_detection_confidence_high = max(self.objects[i]['conf']) > confidence_threshold
                    ...
                    if is_target_object and (is_similarity_high or is_detection_confidence_high) and not self.objects[i]['reperception_visited']:
                        if self.objects[i]['llm_verified'] or self.objects[i]['llm_verified'] is None:
                            ...
                            if self.args.real_world:
                                # relax the threshold for real world
                                candidate_objects.append(self.objects[i])
            ...
            for i in range(len(self.objects)):
                ...
                stiar_threshold = 0.24
                if self.args.real_world:
                    stiar_threshold = 0.0
                    
                if stairs_similarities[i] > stiar_threshold and obj_name == "stairs":
                    if self.args.real_world:
                        # real world stair climbing currently only use upstair case, consider all floor as upstair
        ...
        if self.args.imagine_nav_planner:
            self.imagine_nav_planner.update_scene_graph(self.objects, self.origins_grid, self.gradient_map, self.image_history)
        ...
        if self.args.real_world and self.found_goal:
            self.real_world_climb = False
            print()
        self.is_same_candidate = False
        self.curr_goal_candidate = None
        ...
        if self.args.real_world and False:
            if len(self.upstair_candidate_objects) > 0 and not self.found_goal:
                self.real_world_climb = True
                self.another_floor = True
                self.start_climbing = True
                self.upstair_flag = True
                print("[REAL WORLD] Found upstair candidate")

        elif (self.another_floor or self.early_stair_climb) and (not self.upstair_flag and not self.downstair_flag):
        ...
        # ------------------------------------------------------------------
        fmm_path_x,fmm_path_z =None,None
        ...
        if self.args.real_world:
            is_near_goal = self.dtg_metric < 2 or len(self.fmm_path) < 4
        ##### first version of distance to goal: hm3d 69？ #####
        ...
        if self.found_goal and not ((self.another_floor or self.early_stair_climb) and (self.upstair_flag or self.downstair_flag) and self.start_climbing) and not self.start_climbing:
            ...
            if(self.args.real_world and self.reach_goal_count >= 1):
                action = 0
            else:
        ...
        if self.args.save_step_data:
            ...
            print(f"[agent] [{'current'}] [{self.episode_label}] step: {self.l_step}, action: {action}, step_time: {self.step_times[-1]}")
        ...
        output_trajectory = self.initial_pose @ np.vstack([fmm_path_x,-fmm_path_z,np.zeros(len(fmm_path_x)),np.ones(len(fmm_path_x))]) 
        return action,output_trajectory[0],output_trajectory[1]
    ...
    def fmm_act(self):
        ...
        if self.args.real_world:
            if relative_angle > self.args.turn_angle:
                ...
                action = 1
            # real_world stair climbing has higher priority and will overwrite other action
            # if self.real_world_climb:
            #     print("[REAL WORLD] Climbing Stair")
            #     action = 1
    ...
    def merge_map(self, update_occupancy_map=False, update_traversible_map=False, update_semantic_map=False):
        ...
        obstacle_mask = cv2.dilate(self.obstacle_map.astype('uint8'), kernel_ellipse) == 1
            ...
            if dist < self.args.collision_threshold:  # Collision
                ...
                for i in range(length):
                    for j in range(width):
                        ...
                        if self.args.real_world:
                            pass # we don't do collision check in real world
                        else:
    ...
    def update_map_gradient(self, camera_position):
        #################################################################
        ...
        ####################################################
        self.obstacle_map, self.explored_map = self.get_new_obstacle_explore_map(all_points, global_height_map, global_gradient_map, camera_position, cut_height=False)
    ...
    def update_map_new(self, point_sum, camera_position, height_diff):
        ...
        if self.args.real_world:
            ...
            mask_obstacle = (all_points[:, 1] >= (camera_position[1] - 0.4)) | \
                            (all_points[:, 1] <= (camera_position[1] - 1.3))
    ...
    def update_map(self, point_sum, camera_position, height_diff):
        ...
        if self.args.real_world:
            ...
            z_lower = -1000.0
            # | (points[:, 1] <= z_lower)
            mask_obstacle = (((points[:, 1] >= z_min)) | (points[:, 1] <= z_lower)) & \
    ...
    def get_transform_matrix(self):
        ...
        if self.args.real_world:
            '''
            Transform the pose of real_world GO1 pose to agent memroy space
            x (forward), y (left), z (up) -> x (forward), y (up), z (right)
            '''
            # get initial pose in real_world
            initial_pose = np.eye(4)
            initial_pose[:3, :3] = self.init_sim_rotation
            initial_pose[:3, 3] = self.init_sim_position

            
            # get current pose in real_world
            curr_pose = np.eye(4)
            curr_pose[:3, :3] = quaternion.as_rotation_matrix(self.rotation)
            curr_pose[:3, 3] = self.position 
            
            # construct coordinate transformation matrix
            real_to_memory = np.eye(4)
            real_to_memory[:3, :3] = np.array([[1, 0, 0],
                                                [0, 0, 1],
                                                [0, -1, 0]])
            
            # relative pose in real_world
            real_pose = np.linalg.inv(initial_pose) @ curr_pose
            
            # somehow needed 
            # additional = R.from_euler('xyz', [180, -90, 0], degrees=True)

            
            # relative pose in memory
            # memory_pose = real_to_memory @ real_pose @ real_to_memory.T
            memory_pose =  real_to_memory @ real_pose @ real_to_memory.T @ self.additional
            O_camera_matrix = memory_pose
            print("[REAL_WORLD] transformed pose:", memory_pose[:3, 3])
            print("[REAL_WORLD] transformed quat:", R.from_matrix(memory_pose[:3, :3]).as_quat())
        else:
            """
            ...
            camera_rotation = quaternion.as_rotation_matrix(self.rotation)
    ...
    def get_semantic_map_vis(self, goal_map):
        ...
        sem_map_vis = np.flipud(sem_map_vis).copy() # flip upsdie down x up, z right 
    ...
    def _visualize(self, map_pred, exp_pred, map_edge, goal_map, text_queries):
        ...
        # other text
        correlation_text = f"[Correlation] obj={self.imagine_nav_planner.object_correlation}, region={self.imagine_nav_planner.region_correlation}, prediction={self.imagine_nav_planner.with_prediction}, caption={self.imagine_nav_planner.caption_method}, fusion={self.imagine_nav_planner.mixing_mode}, exploration={self.imagine_nav_planner.exploration_mode}"
        ...
        if (self.another_floor or self.early_stair_climb) and (self.upstair_flag or self.downstair_flag) and not self.args.no_stair_climbing:
            ...
            if hasattr(self, "global_height_image") and hasattr(self, "global_height_gradient_img"):
                vis_grid.append({
                    "type": "image_row",
                    "height": self.local_h,
                    "image": [
        ...
        if self.args.print_images:
            ...
            cv2.imwrite(f'{self.dump_dir}vis/current.png', vis_image)
        if self.args.real_world:
            # # set up save directory
            annotated_dir = f"{self.dump_dir}{self.episode_label}"
            os.makedirs(annotated_dir, exist_ok=True)
            # data directories
            rgb_dir = os.path.join(annotated_dir, "rgb")
            map_dir = os.path.join(annotated_dir, "map")
            os.makedirs(rgb_dir, exist_ok=True)
            os.makedirs(map_dir, exist_ok=True)
            # png file path
            rgb_path = os.path.join(rgb_dir, f"{self.l_step:03}.png")
            map_path = os.path.join(map_dir, f"{self.l_step:03}.png")
            # save data
            cv2.imwrite(rgb_path, self.annotated_image)
            cv2.imwrite(map_path, sem_map_vis)
            pass
        return vis_image
    ...
    def stair_climbing_policy(self, stair_candidate_list, camera_position):
        ...
        ##########################################
        target_score, target_edge_map, target_point_list = detect_frontier(self.explored_map, self.obstacle_map, self.current_grid_pose, threshold_point=8, real_world=self.args.real_world,visited_mask=self.visited_vis)
    ...
    def create_global_height_map(self, point_cloud, height_min):
        ...
        unexplored_area = np.ones((self.local_w, self.local_h),dtype=np.uint8)
        unexplored_area[i_values, j_values] = 0
        # pad the unexplored area with 1 more cell, which is 3x3 filtering
        unexplored_area = cv2.dilate(unexplored_area, np.ones((3, 3), dtype=np.uint8)) # used to mark the edge of the explored area to 0 gradient
        ...
        # set the gradient of the unexplored area to zero
        global_gradient[unexplored_area == 1] = 0
    ...
    def get_new_obstacle_explore_map(self, all_points, global_height_map, global_gradient_map, camera_position, cut_height=False):
        explored_map = np.zeros((self.local_w, self.local_h))
        obstacle_map = np.zeros((self.local_w, self.local_h))
        ...
        # remove small holes inside the region 
        kernel = np.ones((5, 5), dtype=np.uint8)
        ...
        # if cut_height:
        #     robot_wrist_height = camera_position[1] - 0.3 # assume the robot wrist is 0.5 meter below the camera
        #     obstacle_map = (explored_map == 1) & ((global_gradient_map > self.args.GRADIENT_THRESHOLD) | (global_height_map - robot_wrist_height > 0))
        # else:
        ...
        obstacle_map = (explored_map == 1) & (global_gradient_map > self.args.GRADIENT_THRESHOLD)
        ...
        return obstacle_map.astype(np.uint8), explored_map
