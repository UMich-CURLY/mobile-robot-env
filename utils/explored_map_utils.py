...
def build_full_scene_pcd(depth, image, hfov, real_world=False):
    ...
    z = masked_depth
    # if(real_world):
    #     z = np.sqrt(masked_depth**2-x**2-y**2)
    ...
    # if real_world, lower the points to mimic a higher camera
    # if real_world:
    #     pass
        # points[:, 1] -= 0.5
...
def detect_frontier(explored_map, obstacle_map, current_pose, threshold_point, real_world=False,visited_mask=None):
    # ------------------------------------------------------------------
    ...
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(3, 3)) # old version
    dis_obstacle_map = obstacle_map
    
    obstacle_map = cv2.dilate(obstacle_map.astype('uint8'), kernel)
        
    kernel = np.ones((5, 5), dtype=np.uint8)
    # show_ex = cv2.inRange(explored_map,0.1,1) # old code, some times buggy
    show_ex = (explored_map == 1).astype(np.uint8)
    ...
    if real_world:
        kernel_rect = cv2.getStructuringElement(cv2.MORPH_RECT,(16, 16))
        visited_mask = cv2.dilate(visited_mask, kernel_rect) == 1
        print(np.sum(visited_mask))
        target_edge[visited_mask] == 0.0
        target_edge = target_edge - visited_mask.astype(np.float32)
        print(f"[REAL_WORLD] frontier mask out area ")
    ...
    if dict_cost:
        ...
        for i, (key, value) in enumerate(dict_cost):
            ...
            Goal_cost_list.append(value)
            if i == 5:
                break

    return  Goal_cost_list, Goal_edge, Goal_point
