...
def vote_class_name(objects):
    for idx, object in enumerate(objects):
        class_name_list = object['class_name']
        if len(class_name_list) == 0:
            caption = "object"
        else:
            counts = Counter(class_name_list)  
            caption = counts.most_common(1)[0][0]
        object['caption'] = caption

def merge_obj2_into_obj1(cfg, obj1, obj2, run_dbscan=True, current_step=-1):
    '''
    ...
    for k in obj1.keys():
        if k not in ['global_id', 'pcd', 'bbox', 'clip_ft', "text_ft", "caption", "node", "bki_ft", "variance", 'llm_verified']:
        ...
        else: # pcd, bbox, clip_ft, text_ft, bki_ft, variance are handled below
            continue
    
    # update caption
    last_caption = obj1['caption']
    vote_class_name([obj1])
    if obj1['caption'] != last_caption:
        obj1['updated_at'] = current_step
    ...
    ##### bki update #####
    ######################
    prior_ft = obj1['bki_ft'][:obj1['clip_ft'].shape[0]]
    prior_var = obj1['bki_ft'][obj1['clip_ft'].shape[0]:-1]
    prior_conf = obj1['bki_ft'][-1]
    new_ft = obj2['bki_ft'][:obj2['clip_ft'].shape[0]]
    new_var = obj2['bki_ft'][obj2['clip_ft'].shape[0]:-1]
    new_conf = obj2['bki_ft'][-1]
    
    # update the mean
    lambda_star = prior_conf + new_conf
    conjugate_mean = (prior_ft * prior_conf + new_ft * new_conf) / (lambda_star + 1e-6)
    
    # update the variance
    diff = new_ft - prior_ft
    E_star = diff**2
    S_star = 0 # for simplicity, we do not record all the feats at each time step
    # not sure if this formula is theoretically correct
    # if both object has a variance in conjugate space, would it be a simple addition?
    conjugate_var = prior_var + new_var + S_star + (lambda_star * new_conf / (lambda_star + 1e-6)) * E_star
    
    obj1['bki_ft'][:obj1['clip_ft'].shape[0]] = conjugate_mean
    obj1['bki_ft'][obj1['clip_ft'].shape[0]:-1] = conjugate_var
    obj1['bki_ft'][-1] = lambda_star
    
    # get posteiror variance using e-op / d-opt
    if lambda_star > 2:
        t_var = (lambda_star + 1) / (lambda_star * lambda_star + 1e-6) * conjugate_var
        covariance = lambda_star / (lambda_star - 2 + 1e-6) * t_var
        final_var = torch.exp(torch.sum(torch.log(covariance))/covariance.shape[0]) # d-opt
        # final_var = torch.max(final_var, torch.tensor(1e-6)) # e-opt
        obj1['variance'] = final_var # unnormalized variance?
    
    obj1_llm_verified = obj1['llm_verified'] 
    obj2_llm_verified = obj2['llm_verified'] 
    if obj1_llm_verified is not None and obj2_llm_verified is not None:
        if obj1_llm_verified != obj2_llm_verified: # True and Flase
            obj1['llm_verified'] = None # reverify 
        else:
            obj1['llm_verified'] = obj1_llm_verified or obj2_llm_verified
    else: # one is None, the other is not , or both are None
        if obj1_llm_verified is not None:
            obj1['llm_verified'] = obj1_llm_verified
        elif obj2_llm_verified is not None:
            obj1['llm_verified'] = obj2_llm_verified
        else: # both None
            obj1['llm_verified'] = None            
    return obj1
...
def gobs_to_detection_list(
    ...
    for mask_idx in range(n_masks):
        ...
        if class_name == 'stairs':
            pass
        # ignore objects if they are not on the same floor
        too_height = np.array(global_object_pcd.points)[:, 1].min() > trans_pose[1, 3] + 1 
        too_low = np.array(global_object_pcd.points)[:, 1].max() < trans_pose[1, 3] - 1.5
        if too_height or too_low:
            continue
        ...
        detected_object = {
            'caption': class_name,                    # initial caption of the object       
            'updated_at' : idx,                             # idx of the image
            ...
            'reperception_visited': False, # Whether this object should be considered during repeception
            'visited': False, # Whether this object should be considered during best_obj
            ...
            # 'bki_ft': 
            'variance': -1, # variance if invalid until we have lambda > 2 in bki_ft
            'llm_verified': None
        }
        # adding bki conjugate parameters
        bki_ft = torch.zeros(detected_object['clip_ft'].shape[0]*2+1, dtype=torch.float32) # mean, var, conf
        bki_ft[-1] = detected_object['conf'][0] + 1 # lambda = conf + time detection, which is 1 here
        bki_ft[:detected_object['clip_ft'].shape[0]] = detected_object['clip_ft']
        detected_object['bki_ft'] = bki_ft
