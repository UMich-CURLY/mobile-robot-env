...
class IsaacEnv:
    def __init__(self):
        ...
        self.planner = Planner(env, env_cfg, args_cli, simulation_app)
    
    def reset(self):
        obs = self.planner.reset()
        return obs
    ...
    def episode_over(self):
        return self.planner.episode_over
    
    def get_agent_state(self):
        return self.planner.get_agent_state()
    
    def step(self, action):
        return self.planner.step(action)
...
class Planner:
    ...
    def reset(self):
        ...
        self.planner_dt = self.env_cfg.decimation * self.env_cfg.sim.dt # 0.2
        self.traj_dt = 1.0
    ...
    def get_agent_state(self):
        ...
        depth['height'] = 1.1
        depth['rotation'] = self.env.unwrapped.scene["robot"].data.root_quat_w[0].detach().cpu().numpy()
        depth['position'] = self.env.unwrapped.scene["robot"].data.root_pos_w[0].detach().cpu().numpy()
    ...
    def step(self):
        ...
        # print("observations keys:", infos['observations'].keys())
        # ['policy', 'proprio', 'critic', 'camera_obs', 'viz_camera_obs', 'depth_obs'])

        # save observations
        depth_image = infos['observations']['depth_obs'][0,0,:,:].clone().detach().cpu().numpy()
        rgb_image = infos['observations']['camera_obs'][0,:,:,:3].clone().detach()
        rgb_image_np = rgb_image.cpu().numpy()
        rgb_image_viz = infos['observations']['viz_camera_obs'][0,:,:,:3].clone().detach()
        rgb_image_viz_np = rgb_image.cpu().numpy()
        rgb_image_viz_np = cv2.rotate(rgb_image_np, cv2.ROTATE_90_CLOCKWISE)
        ...
        if yaw_diff < -np.pi:
            ...
            yaw_diff -= 2 * np.pi
        target_yaw_rate = np.clip(0.1 * yaw_diff / self.planner_dt, -0.5, 0.5)
