...
def merge_detections_to_objects(
    ...
    for i in range(agg_sim.shape[0]):
        ...
        if agg_sim[i].max() == float('-inf'):
            detection_list[i]['global_id'] = object_id_count['count']
            object_id_count['count'] += 1
        ...
        # Merge with most similar existing object
            ...
            # update updated_at if object caption is changed
            merged_obj = merge_obj2_into_obj1(cfg, matched_obj, matched_det, run_dbscan=False, current_step=objects.get_updated_at())
