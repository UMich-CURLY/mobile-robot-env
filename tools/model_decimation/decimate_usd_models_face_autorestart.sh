...
## Usage: ./decimate_usd_models_face_autorestart.sh "$HOME/data/isaac_scenes_v1/grscenes_commercial/models/"
# pkill -9 -f "python.*decimate_usd_models_face_blender.py"

# Check if input folder argument is provided
if [ $# -eq 0 ]; then
    echo "Usage: $0 <input_folder>"
    echo "Example: $0 $HOME/data/isaac_scenes_v1/grscenes_commercial/models/"
    exit 1
fi
...
run_worker() {
    ...
    while true; do
        ...
        systemd-run --user --scope -p MemoryMax=$MEM_LIMIT blender --background --python "$SCRIPT" -- \
            --input_folder "$INPUT_FOLDER" --ratio $RATIO --worker_id $WORKER_ID --total_workers $TOTAL_WORKERS >>"$LOG" 2>&1 &
        local PID=$!
