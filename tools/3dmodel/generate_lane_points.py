...
def get_lane_width(tags, highway):
    ...
    lanes = None
    try:
        lanes = int(tags.get("lanes")) if tags.get("lanes") else None
    except (ValueError, TypeError):
        lanes = None
...
def build_lane_segments(nodes, ways):
    segments = []
    for way in ways:
        tags = way.get("tags", {})
        if not is_lane_way(tags):
            continue
        highway = tags.get("highway")
        width = get_lane_width(tags, highway)
...
def process_city(city, n_points, seed):
    ...
    segments = build_lane_segments(nodes, ways)
    if not segments:
        print(f"Skip {city}: no lane segments found")
        return None
    ...
    return {
        "city": city,
        "input": osm_file,
        "lane_segments": len(segments),
        "lane_length_m": total_length,
        "points": points,
    }
...
def main():
    parser = argparse.ArgumentParser(description="Generate random points on car lanes.")
    ...
    for city in city_list:
        ...
        out_path = os.path.join(out_dir, "lane_points.json")
