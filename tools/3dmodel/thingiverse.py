...
class ThingiverseDownloader(ObjaverseSource):
    ...
    def get_annotations(
        ...
        if refresh or not fs.exists(path):
            ...
            response = requests.get(remote_url, impersonate="chrome110")
    ...
    def _get_response_with_retries(
        ...
        """
        # Default user agent (Windows Chrome)
        default_ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        
        headers = {
            "User-Agent": os.environ.get("THINGIVERSE_USER_AGENT", default_ua)
        }
        
        if "THINGIVERSE_COOKIE" in os.environ:
            headers["Cookie"] = os.environ["THINGIVERSE_COOKIE"]
    ...
    def get_alignment_annotations(
        ...
        Returns:
            pd.DataFrame: Metadata of the 3D objects as a Pandas DataFrame with columns
                ...
                "sha256", and "metadata".
        """
        logger.info("Thingiverse objects weren't used during alignment annotations")
        return pd.DataFrame()
