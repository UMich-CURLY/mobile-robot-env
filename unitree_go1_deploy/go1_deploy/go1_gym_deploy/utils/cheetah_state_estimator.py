...
class StateEstimator:
    ...
    def get_command(self):
        ...
        # always in use
        cmd_x = 0.5 * self.left_stick[1]
        cmd_yaw = 2 * self.right_stick[0]

        # default values
        cmd_y = -1 * self.left_stick[0]  # -1 * self.left_stick[0]
        cmd_height = 0.
        cmd_footswing = 0.08
    ...
    def _imu_cb(self, channel, data):
        ...
        self.contact_state = 1.0 * (np.array(msg.contact_estimate) > 10)
        # print(msg.contact_estimate)
    ...
    def _rc_command_cb(self, channel, data):
        ...
        if time.time() - self._last_relay_time > 0.5:
