...
def swap_all_decimated_meshes_in_folder(models_folder):
    ...
    for root, dirs, files in os.walk(models_folder):
        # Only process folders containing 'instance.usd' and 'instance_decimated.usd'
        if 'instance_decimated.usd' in files and 'instance.usd' in files:
            original_usd = os.path.join(root, 'instance.usd')
            decimated_usd = os.path.join(root, 'instance_decimated.usd')
            
            if os.path.exists(decimated_usd):
                output_usd = original_usd  # overwrite original
                print(f"[INFO] Swapping meshes: {original_usd} -> {decimated_usd}")
...
if __name__ == "__main__":
    ...
    # Params:
    input_folder = "/home/junzhewu/pohsun/data_decimated/grscenes_commercial/models_decimated2"
